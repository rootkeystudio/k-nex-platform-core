...
Source clients treat realtime revisions only as hints. Their executable lifecycle attaches invalidation, reconnect, and focus signals and schedules its own bounded periodic revalidation. It performs authoritative authorization and fetch on initialization, newer hints, reconnect, focus, and periodic ticks. Refresh callbacks receive an abort signal and a strict deadline; timeout clears cached private data, late success or rejection cannot overwrite a fail-closed state or a newer successful retry, and a later cycle retries after the dependency recovers. A lost hint converges automatically; lagging snapshots remain stale; revision regression cannot overwrite newer data; revocation or authorization failure clears cached private data.
