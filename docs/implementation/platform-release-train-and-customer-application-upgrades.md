...
The exact frozen contract is [`platform-release-transition-manifest.v1.schema.json`](../../schemas/platform-release-transition-manifest.v1.schema.json), generated from `PlatformReleaseTransitionManifestSchema`. This plan does not restate or extend that schema.

It binds immutable source and target releases and manifest digests, direct-predecessor policy, exact package mappings, generator/framework/schema compatibility, migration and delivery classifications, backup/restore policy, and lifecycle/revocation/expiry metadata. The contract is closed and bounded; executable migration source, repository credentials, image pull secrets, arbitrary URLs, and browser-selected digests are excluded.
...
The ownership manifest digest flows one way into this release lock, and this release-lock digest flows one way into the upgrade input snapshot. Neither an ownership manifest nor a release lock may contain its own digest, a digest of a containing envelope, or a reverse pointer. Cyclic or recursively derived digest graphs are invalid.
...
Only `managed`, `append-only`, and `template` records appear. Absence means `customer/unowned`; it is never upgraded by inference. The ownership manifest excludes release-control paths: `k-nex.app.json`, `package.json`, `pnpm-lock.yaml`, `.k-nex/release-lock.json`, `.k-nex/generated-files.json`, packed-artifact metadata, and upgrade plan/receipt files. Those controls update only through their separately verified release-control procedure.

It contains no absolute path, timestamp, hostname, secret, uncommitted state, current-file bytes, merge base, or reconciliation trace. Reconciliation evidence is implementation-private: it can support a prepared source change and verified receipt, but is not a customer-authored, executable, or recursive persisted ownership input.

The first executable contract is intentionally minimal:

```ts
interface ApplicationUpgradePlanEnvelopeV1 {
  schemaVersion: 1;
  planId: string;
  inputSnapshotDigest: `sha256:${string}`;
  targetTransitionManifestDigest: `sha256:${string}`;
  operations: readonly (
    | { path: string; kind: "add" | "replace" | "append" | "conflict" }
    | { path: string; kind: "blocked"; reason: "managed-delete-reference-proof-required" }
  )[];
  digest: `sha256:${string}`;
}

interface PreparationResultV1 {
  schemaVersion: 1;
  planDigest: `sha256:${string}`;
  preparedSourceCommit: string;
  preparedReleaseLockDigest: `sha256:${string}`;
  resultDigest: `sha256:${string}`;
}
```

Each digest covers canonical JSON of its own fields excluding that digest field. `inputSnapshotDigest` covers the immutable source inputs and embeds neither plan/result digest nor reconciliation evidence. The release lock may bind `sourceOwnershipDigest`; the input snapshot may bind the release-lock digest; no later artifact may feed back into either earlier digest.
...
| target no longer emits and current digest equals prior | managed | block `managed-delete-reference-proof-required` |
...
| customer/unowned file occupies target path | absent | conflict |
| template already exists | template | preserve; expose compare/adopt if supported |

No hidden merge is attempted. Successful managed deletion is deferred beyond PR #35. The plan identifies supported override points to move customer behavior out of a managed file.
...
13. emit canonical source-change plan and commit/PR; retain reconciliation evidence implementation-private
```
...
managed/add/blocked-delete/conflict plan
...
- managed deletion is explicitly blocked with `managed-delete-reference-proof-required` in PR #35;
