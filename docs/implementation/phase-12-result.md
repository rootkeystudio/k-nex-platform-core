...
Post-review focused evidence rebuilt the exact v1 packed closure and passed the generated PostgreSQL/HTTP/Chromium journey with published-only navigation, durable server-scoped sidebar reload, current-authority folder mutation, fixed-System-parent denial, one-winner folder CAS, built-in/Sales blocks, and Sales disable/recovery. This does not close the System administration blocker or replace the required exact-head cumulative gate.
