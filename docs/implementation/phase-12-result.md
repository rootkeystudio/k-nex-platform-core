...
The final response-loss closure adds one signed server-owned plan intent, atomically binds the exact execute-command digest inside the terminal lifecycle transaction, and rejects completed rows without that binding. Real TLS 1.3 mTLS proofs cover dropped responses, cross-actor poisoning denial, pre-mutation operator crash/restart, post-commit operator crash/exact replay, changed-command denial, and exactly one terminal receipt, audit event, and outbox event. Hosted release-evidence run `33944370651` refreshed both customer bundles, five attestations, verification records, runtime inventories, receipts, restore proofs, and fleet evidence for exact closure source `c8fe7f2518c219957297155e307768837186ec5f`.
