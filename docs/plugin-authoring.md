...
[Sales UI bindings](../modules/sales/src/ui.ts) separate canonical descriptors from browser renderers. Every component renders outside the editor. Runtime registries and Puck snapshots rebind descriptor-bearing definitions from their parsed descriptor, so only the renderer remains executable caller authority; supplied props validators and source/action policies cannot replace serialized plugin policy. Puck reconciles the same renderer and persisted props rather than owning a parallel component model. Default pages are immutable package templates; [the page-template runtime](../packages/runtime/src/page-template.ts) creates customer-owned instances idempotently, preserves edits, and requires explicit compare/adopt for later versions. Inventory authority must expose a nonnegative safe revision, and store replacement must atomically compare that revision with the customer page revision after adoption re-preflights a fresh immutable snapshot.
...
[The lifecycle runtime](../packages/runtime/src/plugin-lifecycle.ts) keeps catalog support, installed bytes, enabled state, settings readiness, migration readiness, data state, and release support independent. Install planning is source-controlled and requires deployment. Disable retains package/schema/data and read compatibility while gating writes, sources, actions, tools, jobs, navigation, routes, components, blocks, and page behavior. Required plugin and capability-provider consumers are removed transitively when their provider is unavailable; optional dependencies do not remove a consumer. Capability services must be objects or functions: registration returns lifecycle leases, which work during registration, require authoritative lifecycle scoping after freeze, and fail after the provider or required consumer is revoked. Async capability results resolve through the same lease and fail closed when availability changes before fulfillment. Re-enable requires current settings and migration readiness. Schema-owning uninstall remains unsupported; destructive work starts with a deterministic reference scan. Archive/export, purge, upgrade, backup, and restore remain Gate 8.
