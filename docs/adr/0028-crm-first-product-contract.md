...
Related-record vocabulary is closed: `sales.account`, `sales.contact`, `sales.lead`, `sales.opportunity`, and `sales.task`. Activities, Notes, Attachment references, and optional Task links share this vocabulary only, and each target resolves in the same application/environment. Intent remains object-specific: Activity is an interaction target, Note is a timeline target, Attachment reference is a storage-reference target, and Task's follow-up target is optional. The product contract, Payload select options, and PostgreSQL trigger must remain byte-for-byte equivalent in vocabulary; no object may introduce another polymorphic target without a new accepted contract decision.
...
Each mutable record has exactly one owner; optional team assignment broadens scope only through current policy. Reassignment requires explicit permission, expected revision, audit, and invalidation. Account/contact relationship never implies access without current record and field authorization. Protected ownership, application, audit, lifecycle, revision, and merge-lineage fields cannot be set by generic create/update/import actions.
...
Fixed record detail pages contain summary, owner/team, relations, activity/note/attachment timeline, tasks/reminders, state history, and authorized actions. Custom Phase 12 pages embed registered saved views and reports only; they do not replace these daily routes.
