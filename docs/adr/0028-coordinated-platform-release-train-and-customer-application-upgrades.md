...
Ownership records describe only generated paths. Absence is `customer/unowned`: it is never inferred as managed, deleted, or rewritten by the upgrade compiler. Release-control files (`k-nex.app.json`, `package.json`, `pnpm-lock.yaml`, `.k-nex/release-lock.json`, `.k-nex/generated-files.json`, packed-artifact metadata, and the prepared-plan/receipt paths) are excluded from ownership reconciliation and update only through their separately verified release-control procedure.
...
3. In PR #35, deletion of a managed path always blocks with `managed-delete-reference-proof-required`; passing reference proof and successful managed deletion are deferred to a later accepted decision and implementation.
