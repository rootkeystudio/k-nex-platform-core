...
test("P13.4 generated HTTP configuration actions commit CAS, audit, outbox, realtime delivery, and idempotency atomically", { timeout: 360_000 }, async () => {
  await withGeneratedCrmBrowserFixture(async ({ origin, personas, records, pool, applicationOutput, runDoctor }) => {
    assert.match(runDoctor(), /K_NEX_APPLICATION_READY/u, "generated doctor must accept the exact system.general@3 reporting timezone document");
    ...
    assert.deepEqual((await pool.query("select document_revision,settings_revision,values_json from k_nex_system_settings_documents where application_id='p13-crm-browser' and environment='test' and descriptor_id='system.general' and descriptor_schema_version=3 and owner_scope_key='platform:system'")).rows[0], generalBeforeUnrelated, "unrelated settings advance leaves system.general revision unchanged");
    ...
    await pool.query("update k_nex_system_settings_documents set values_json=jsonb_set(values_json,'{reportingTimezone}','\"Mars/Olympus\"'::jsonb) where application_id='p13-crm-browser' and environment='test' and descriptor_id='system.general' and descriptor_schema_version=3 and owner_scope_key='platform:system'");
    ...
    assert.throws(() => runDoctor(), /Application reporting settings v3 readiness mismatch/u, "doctor rejects malformed system.general reporting timezone");
    await pool.query("update k_nex_system_settings_documents set values_json=$1::jsonb where application_id='p13-crm-browser' and environment='test' and descriptor_id='system.general' and descriptor_schema_version=3 and owner_scope_key='platform:system'", [JSON.stringify(generalBeforeUnrelated.values_json)]);
    await pool.query("update k_nex_system_settings_documents set settings_revision=$1 where application_id='p13-crm-browser' and environment='test' and descriptor_id='system.general' and descriptor_schema_version=3 and owner_scope_key='platform:system'", [unrelatedStateRevision + 1]);
    ...
    assert.throws(() => runDoctor(), /Application reporting settings v3 readiness mismatch/u, "doctor rejects future system.general settings revision");
    await pool.query("update k_nex_system_settings_documents set settings_revision=$1 where application_id='p13-crm-browser' and environment='test' and descriptor_id='system.general' and descriptor_schema_version=3 and owner_scope_key='platform:system'", [generalBeforeUnrelated.settings_revision]);
    ...
    const settingsDocument = (await pool.query("select document_revision,settings_revision,values_json from k_nex_system_settings_documents where application_id='p13-crm-browser' and environment='test' and descriptor_id='system.general' and descriptor_schema_version=3 and owner_scope_key='platform:system'")).rows[0];
