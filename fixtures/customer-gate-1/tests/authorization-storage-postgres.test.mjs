...
test("migrates P10.3 authorization storage with customer isolation and generation fences", { timeout: 180_000 }, async () => {
  ...
  try {
    ...
    assert.deepEqual((await pool.query("select predecessor_revision, revision from k_nex_migration_revision where id=1")).rows, [{ predecessor_revision: 24, revision: 25 }]);
