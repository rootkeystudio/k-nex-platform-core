...
import {
  CurrentAuthorityAdapter, EffectiveAuthorityResolver, SystemOperationsAdministrationService, SystemSettingsAdministrationService,
...
test("P11.8 serves fixed settings, theme, and operations journeys with current authority", { timeout: 180_000 }, async () => {
  ...
  try {
    ...
    const settings = new SystemSettingsAdministrationService({ authority, state: authorizationStore, store: settingsStore, descriptorSource: { list: async () => [{ descriptor, identity: { applicationId, environment, descriptorId: descriptor.id, descriptorSchemaVersion: 1, owner: { kind: "platform", namespace: "system" } }, lifecycle: "active" }] }, metadata: (() => { let id = 0; return { id(kind) { id += 1; return `settings-${kind}-${id}`; }, now() { return new Date("2026-09-02T00:00:00.000Z"); } }; })() });
