...
test("P10.9 proves fixed host routes, RBAC actions, lifecycle truth, and Chromium semantics against PostgreSQL", { timeout: 180_000 }, async () => {
  ...
  try {
    ...
    let salesAssistantRevision = 4;
    let currentSalesAssistantOperationId;
    ...
    const operator = {
      ...
      async status() { operatorCalls += 1; return { applicationId, environment, inventory: { revision: inventoryRevision, extensions: {
        hotApplications: { "app.sales-assistant": { disposition: "active", revision: salesAssistantRevision, ...(currentSalesAssistantOperationId === undefined ? {} : { lastOperationId: currentSalesAssistantOperationId }) } },
      ...
      async plan(input) { operatorCalls += 1; const current = operations.get(input.idempotencyKey) ?? operation(input); if (!operations.has(input.idempotencyKey)) { operations.set(input.idempotencyKey, current); if (input.extension.id === "app.sales-assistant") { inventoryRevision += 1; salesAssistantRevision += 1; currentSalesAssistantOperationId = current.operationId; } } return current.plan; }, async operation(operationId) { operatorCalls += 1; return [...operations.values()].find((current) => current.operationId === operationId) ?? operation({ applicationId, environment, extension: { deliveryClass: "hot-application", id: "app.sales-assistant" }, operation: "install", targetVersion: "1.0.0", expectedRevision: salesAssistantRevision, idempotencyKey: "system-administration-plan" }); },
    ...
    const hostExtensions = Object.create(extensions);
    hostExtensions.plan = async (input) => {
      const result = await extensions.plan(input);
      if (input.request?.extension?.id === "app.sales-assistant") Object.assign(input.expected, { inventoryRevision, extensionRevision: salesAssistantRevision });
      return result;
    };
    host = await startSystemAdministrationHost({ access, extensions: hostExtensions, context(requestMessage) { const token = /(?:^|;\\s*)system_session=([^;]+)/u.exec(requestMessage.headers.cookie ?? "")?.[1]; return token ? { session: token } : undefined; }, sessionKey(context) { return context?.session; }, async expected(extension) { const state = await store.readState(applicationId, environment); assert.ok(state); const revision = extension?.id === "app.sales-assistant" ? salesAssistantRevision : extension?.id === "skin.accent" ? 5 : extension?.id === "module.reports" ? 6 : extension?.id === "module.sales" ? 7 : 0; return { ...state, inventoryRevision, extensionRevision: revision }; } });
    ...
    const expected = async (extension) => {
      ...
      assert.ok(state);
      const revision = extension?.id === "app.sales-assistant" ? salesAssistantRevision : extension?.id === "skin.accent" ? 5 : extension?.id === "module.reports" ? 6 : extension?.id === "module.sales" ? 7 : 0;
      return { applicationId, environment, authorizationRevision: state.authorizationRevision, lifecycleRevision: state.lifecycleRevision, inventoryRevision, extensionRevision: revision };
    };
    ...
    const plannedSalesAssistant = { deliveryClass: "hot-application", id: "app.sales-assistant" };
    const plannedOperationId = "system-plan-appsalesassistant";
    currentSalesAssistantOperationId = "superseding-operation";
    assert.equal((await request(host.url, "/api/system/extensions/app.sales-assistant/execute", "owner", { expected: await expected(plannedSalesAssistant), operationId: plannedOperationId })).status, 409, "superseded operation ownership must conflict before lifecycle evidence is evaluated");
    currentSalesAssistantOperationId = plannedOperationId;
    await actionPage.goto(`${host.url}/system/extensions/app.sales-assistant`);
