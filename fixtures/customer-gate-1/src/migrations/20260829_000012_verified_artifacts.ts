...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
    CREATE TABLE "runtime_extension_artifacts" (
      ...
      CONSTRAINT "runtime_extension_artifacts_class_check" CHECK ("delivery_class" = 'hot-application'),
      CONSTRAINT "runtime_extension_artifacts_bytes_check" CHECK (octet_length("artifact_bytes") BETWEEN 1 AND 268435456 AND octet_length("provenance_bytes") BETWEEN 1 AND 1048576),
      CONSTRAINT "runtime_extension_artifacts_catalog_check" CHECK (jsonb_typeof("catalog_json") = 'object')
    );
    ...
    CREATE TABLE "runtime_extension_artifact_bindings" (
      ...
      CONSTRAINT "runtime_extension_artifact_bindings_authority_check" CHECK (jsonb_typeof("authority_json") = 'object'),
