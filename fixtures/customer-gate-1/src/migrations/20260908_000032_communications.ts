...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  await db.execute(sql.raw(`CREATE TABLE sales_provider_configurations (
    application_id text NOT NULL, environment text NOT NULL, provider_id text NOT NULL, secret_reference text NOT NULL, webhook_secret_reference text NOT NULL,
    ...
    CHECK(provider_id IN ('email.reference.v1','calendar.reference.v1') AND length(secret_reference) BETWEEN 1 AND 512 AND secret_reference !~ '[[:space:]]' AND length(webhook_secret_reference) BETWEEN 1 AND 512 AND webhook_secret_reference !~ '[[:space:]]' AND webhook_secret_reference<>secret_reference AND secret_reference LIKE '%:provider-api:%' AND webhook_secret_reference LIKE '%:webhook-signature:%' AND revision BETWEEN 1 AND 1000000000 AND state IN ('active','revoked') AND jsonb_typeof(audit)='array' AND (state='active')=(revoked_at IS NULL))
  );
  ...
  CREATE TABLE sales_provider_operations (
    ...
    effect_claim_id text, effect_dispatched_at timestamptz, provider_receipt_id text, provider_receipt_at timestamptz,
  ...
  CREATE TABLE sales_provider_activity_receipts (
    operation_id text PRIMARY KEY REFERENCES sales_provider_operations(operation_id) ON DELETE RESTRICT, activity_id integer NOT NULL UNIQUE REFERENCES sales_activities(id) ON DELETE RESTRICT, receipt_digest text NOT NULL, provider_receipt_id text NOT NULL,
    CHECK(receipt_digest ~ '^sha256:[0-9a-f]{64}$' AND provider_receipt_id ~ '^[A-Za-z0-9][A-Za-z0-9._:-]{7,159}$')
  );
