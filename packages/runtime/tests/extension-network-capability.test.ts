...
const context = {
  ...
  correlationId: "network-correlation-1", grants: [
    { kind: "http-fetch" as const, required: true, reason: "Fetch sales tasks.", destinations: ["https://api.example.test"], methods: ["GET" as const] },
    { kind: "secret-reference" as const, required: true, reason: "Authenticate sales task requests.", references: ["sales.api"] }
  ], issuedAt: "2026-08-29T10:00:00.000Z", expiresAt: "2026-08-29T10:01:00.000Z"
};
...
describe("bounded extension network capability", () => {
  it("keeps secret values inside the host adapter and binds requests to exact allowlists", async () => {
    ...
    const capability = new BoundedExtensionNetworkCapability(
      { destinations: ["https://api.example.test", "https://other.example.test"], methods: ["GET", "POST"], secretReferences: ["sales.api", "other.api"] },
  ...
  it("intersects host policy with token destinations, methods, and secret references", async () => {
    const capability = new BoundedExtensionNetworkCapability(
      { destinations: ["https://api.example.test", "https://other.example.test"], methods: ["GET", "POST"], secretReferences: ["sales.api", "other.api"] },
    ...
    await expect(capability.invoke(context, capability.validateInput({ destination: "https://other.example.test", path: "/", method: "GET", headers: {} }), new AbortController().signal)).rejects.toMatchObject({ code: "DESTINATION_DENIED" });
