...
const currentReleaseManifest = {
  ...
  release: { ...targetReleaseManifest.release, version: "0.1.0" },
  packages: [{ ...targetReleaseManifest.packages[0], version: "0.9.0", integrity: `sha512-${"b".repeat(86)}==` }],
  supportWindow: { ...targetReleaseManifest.supportWindow, supportedReleases: ["0.1.0"] }
} as const satisfies PackageReleaseManifest;
const support = { currentPlatformRelease: "0.1.0", targetPlatformRelease: "0.2.0", currentReleaseManifest, targetReleaseManifest } as const;
...
describe("plugin upgrade planner", () => {
  it("orders the customer migration before every Sales artifact migration and dry-runs all evolution domains", () => {
    const plan = planPluginUpgrade({ ...support, pluginId: "module.sales", currentVersion: "0.9.0", targetVersion: "1.0.0", targets, migrations });
  ...
  it("fails preflight on gaps, duplicate revisions, invalid predecessors, and dependency cycles", () => {
    const gap = planPluginUpgrade({ ...support, pluginId: "module.sales", currentVersion: "0.9.0", targetVersion: "1.0.0", targets: [targets[0]!], migrations: [] });
    ...
    const invalid = planPluginUpgrade({
      ...support, pluginId: "module.sales", currentVersion: "0.9.0", targetVersion: "1.0.0", targets: [targets[0]!],
    ...
    const cycle = planPluginUpgrade({ ...support, pluginId: "module.sales", currentVersion: "0.9.0", targetVersion: "1.0.0", targets: [targets[0]!], migrations: [cycleStep] });
  ...
  it("reports dry-run migration failures without changing its input", () => {
    ...
    const plan = planPluginUpgrade({ ...support, pluginId: "module.sales", currentVersion: "0.9.0", targetVersion: "1.0.0", targets: [targets[0]!], migrations: broken });
  ...
  it("refuses stale dry-run artifacts and unknown graph dependencies", () => {
    ...
    const invalid = planPluginUpgrade({ ...support, pluginId: "module.sales", currentVersion: "0.9.0", targetVersion: "1.0.0", targets: [targets[0]!], migrations: unknownDependency });
    expect(invalid).toMatchObject({ ready: false, diagnostics: expect.arrayContaining([expect.objectContaining({ code: "INVALID" })]) });

    const plan = planPluginUpgrade({ ...support, pluginId: "module.sales", currentVersion: "0.9.0", targetVersion: "1.0.0", targets: [targets[0]!], migrations: [migrations[0]!] });
  ...
  it("rejects a platform source outside the current release support window", () => {
    const plan = planPluginUpgrade({ ...support, currentPlatformRelease: "0.0.1", pluginId: "module.sales", currentVersion: "0.9.0", targetVersion: "1.0.0", targets, migrations });
  ...
  it("rejects invented plugin target versions outside the trusted release artifact", () => {
    const plan = planPluginUpgrade({ ...support, pluginId: "module.sales", currentVersion: "0.9.0", targetVersion: "9.9.9", targets, migrations });
    expect(plan).toMatchObject({ ready: false, diagnostics: expect.arrayContaining([expect.objectContaining({ code: "UNSUPPORTED_RELEASE" })]) });
  });
});
