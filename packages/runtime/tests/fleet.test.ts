...
import {
  FleetRegistry, createDeploymentEvidenceAuthority, createDeploymentReceipt, createPackageReleaseManifestAuthority, observeRuntimeInventory,
...
const authority = () => createDeploymentEvidenceAuthority({ releaseVerifier, packageReleaseAuthority: releaseAuthority, deploymentPublicKey: publicPem(deploymentKeys.publicKey), trustedDeploymentWorkflow: deploymentWorkflow });
...
const priorManifest = {
  ...
  packages: [{ ...supportManifest.packages[0], version: "0.9.0", integrity: `sha512-${"b".repeat(86)}==` }],
...
async function deployment(applicationId: "customer-alpha" | "customer-beta", release: "0.2.1" | "0.2.0" | "0.1.0", trusted: DeploymentEvidenceAuthority, deploymentNumber = 1, deployedAt = "2026-08-27T12:05:00.000Z") {
  ...
  const attestation = {
    subjectDigest: digest("1"), sourceCommit: sha, workflowIdentity: releaseWorkflow,
  ...
  const inventory = observeRuntimeInventory({
    ...
    plugins: [{ id: "module.sales", package: "@k-nex/module-sales", version: release === "0.1.0" ? "0.9.0" : release === "0.2.1" ? "1.0.1" : "1.0.0", enabled: true }], migrationRevision: release === "0.1.0" ? 6 : release === "0.2.1" ? 8 : 7,
  ...
  const evidence = await trusted.verify({
    ...
    releaseAttestation: issueAttestation(attestation), packageRelease
  });
...
describe("fleet evidence and patch propagation", () => {
  it("ingests only authority-verified deployments and preserves current/prior release state", async () => {
    ...
    const fleet = new FleetRegistry(await verifiedManifest(supportManifest), releaseAuthority, trusted);
  ...
  it("orders RFC3339 deployment timestamps by instant across offsets", async () => {
    ...
    const regression = new FleetRegistry(await verifiedManifest(supportManifest), releaseAuthority, trusted);
  ...
  it("finds every vulnerable deployment and creates customer-specific patch updates", async () => {
    ...
    expect(fleet.planSecurityPatch("@k-nex/module-sales", "<1.0.1", "1.0.1", await verifiedManifest(patchManifest))).toEqual([
  ...
  it("applies only an issued full-closure transition with a fresh verified observation", async () => {
    ...
    const fleet = new FleetRegistry(await verifiedManifest(supportManifest), releaseAuthority, trusted);
    fleet.ingest((await deployment("customer-alpha", "0.2.0", trusted)).evidence);
    const [plan] = fleet.planSecurityPatch("@k-nex/module-sales", "<1.0.1", "1.0.1", await verifiedManifest(patchManifest));
    expect(plan?.targetClosure).toEqual(patchManifest.packages.map(({ package: packageName, version, integrity }) => ({ package: packageName, version, integrity })));
