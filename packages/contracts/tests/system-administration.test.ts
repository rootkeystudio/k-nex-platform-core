...
describe("P11.1 system administration contracts", () => {
  ...
  it("models operations as projection-bound requests/receipts and health as derived observations", () => {
    ...
    const receipt = { schemaVersion: 1, receiptId: "operations-receipt-1", requestId: request.requestId, kind: "backup", applicationId: request.applicationId, environment: request.environment, expectedInventoryDigest: request.expectedInventoryDigest, requestedBy: request.requestedBy, authorityDigest, executionAuthority: request.executionAuthority, idempotencyKey: request.idempotencyKey, reference: { ...request.reference, receiptId: "backup-receipt-1" }, outcome: "completed", reason: "completed", occurredAt: timestamp } as const;
