...
import { createThemeSkinCss, type ResolvedThemeSkin, type ThemeSkinRegistry } from "./skin.js";

export { themeRootSelector } from "./theme-css.js";
...
export interface ResolvedThemeProfile {
  ...
  readonly values: ThemeTokenValues;
  readonly skin?: ResolvedThemeSkin;
}
...
export interface ThemePresentationSnapshot {
  ...
  readonly skinRecipes: Readonly<Record<string, string>>;
  readonly skinAssetHandles: Readonly<Record<string, string>>;
}
...
function snapshotThemePackage(input: ThemePackage): ThemePackage {
  ...
  const structuralCss = scopeThemeCss(input.structuralCss);
...
export function createThemeRegistry(inputs: readonly ThemePackage[], skins?: ThemeSkinRegistry) {
  ...
  return Object.freeze({
    ...
    resolveProfile(value: unknown): ResolvedThemeProfile {
      ...
      const skin = profile.skin === undefined ? undefined : skins?.resolve(profile.skin);
      if (profile.skin !== undefined && skin === undefined) throw new TypeError("Theme Skin generation registry is unavailable.");
      return Object.freeze({ package: themePackage, profile: deepFreeze(structuredClone(profile)), values: cloneValues(result.data), ...(skin ? { skin } : {}) });
    }
  });
}
...
export function createThemePresentation(resolved: ResolvedThemeProfile): ThemePresentationSnapshot {
  ...
  const skinCss = resolved.skin ? createThemeSkinCss(resolved.skin, resolved.profile.revision.id) : "";
  const skinRecipes = Object.freeze(Object.fromEntries(Object.entries(resolved.skin?.generation.manifest.recipes ?? {}).filter((entry): entry is [string, string] => entry[1] !== undefined)));
  return Object.freeze({
    ...
    cssVariables,
    cssText: `${selector}{${declarations}}${resolved.package.structuralCss.replaceAll(themeRootSelector, selector)}${skinCss}`,
    primitives: createSemanticPrimitives(resolved.package.primitiveOverrides),
    ...(resolved.skin ? { skinId: resolved.skin.generation.manifest.id, skinGenerationId: resolved.skin.generation.generationId } : {}),
    skinRecipes,
    skinAssetHandles: Object.freeze({ ...(resolved.skin?.generation.assetHandles ?? {}) })
  });
}
