...
const implementation = join(packageRoot, "src/aria.tsx");
for (const root of sourceRoots) {
  for (const extension of [".ts", ".tsx"]) {
    for (const file of await filesUnder(root, extension)) {
      if (file !== implementation) assert(!/from\s+["']react-aria-components(?:\/[^"']*)?["']/.test(await readFile(file, "utf8")), `React Aria import escaped its K-Nex primitive boundary: ${file}`);
    }
  }
}
