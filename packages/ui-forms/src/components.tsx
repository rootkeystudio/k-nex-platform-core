...
export function Checkbox(props: CheckboxProps): ReactElement {
  const ids = useFieldIds(props.name);
  return <div data-k-nex-component="checkbox" data-slot="root" data-state={props.checked ? "selected" : props.disabled ? "disabled" : "default"}><label htmlFor={ids.input}><input id={ids.input} name={props.name} type="checkbox" checked={props.checked} required={props.required} disabled={props.disabled} onChange={(event) => props.onChange(event.currentTarget.checked)} data-slot="control" />{props.label}</label>{props.description === undefined ? null : <FieldDescription>{props.description}</FieldDescription>}{props.error === undefined ? null : <FieldError>{props.error}</FieldError>}</div>;
}
...
export function DateRangePicker(props: DateRangePickerProps): ReactElement {
  return <fieldset disabled={props.disabled} data-k-nex-component="date-range-picker" data-slot="root"><legend>{props.label}</legend><input name={`${props.name}-start`} type="date" value={props.start} onChange={(event) => props.onChange({ start: event.currentTarget.value, end: props.end })} aria-label="Start date" data-slot="start" /><input name={`${props.name}-end`} type="date" value={props.end} onChange={(event) => props.onChange({ start: props.start, end: event.currentTarget.value })} aria-label="End date" data-slot="end" />{props.error === undefined ? null : <FieldError>{props.error}</FieldError>}</fieldset>;
}
