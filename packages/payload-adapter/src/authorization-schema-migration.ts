...
/** Clean-database v1 schema for current authorization, session invalidation, and owner bootstrap. */
export const kNexAuthorizationSchemaMigration = Object.freeze({
  name: "20260903_000003_knex_authorization",
  async up({ db }: MigrateUpArgs): Promise<void> {
    await db.execute(sql`
      ...
      CREATE TABLE "k_nex_extension_authorization_generations" (
        ...
        CONSTRAINT "k_nex_extension_authorization_generations_state_check" CHECK ("state" IN ('pending-configuration', 'current', 'retired') AND ("state" <> 'pending-configuration' OR ("delivery_class" = 'hot-application' AND jsonb_array_length("runtime_generation_ids") = 1))),
      ...
      CREATE TABLE "k_nex_role_permission_grants" (
        ...
        CONSTRAINT "k_nex_role_permission_grants_owner_check" CHECK (
          ...
          OR ("owner_kind" = 'extension' AND "owner_namespace" IS NULL AND "owner_delivery_class" IN ('platform-plugin', 'hot-application') AND "owner_extension_id" IS NOT NULL AND "owner_generation" BETWEEN 1 AND 9007199254740991 AND "permission_id" LIKE substring("owner_extension_id" FROM position('.' IN "owner_extension_id") + 1) || '.%')
        )
      );
      ...
      CREATE TABLE "k_nex_role_template_adoptions" (
        ...
        CONSTRAINT "k_nex_role_template_adoptions_owner_check" CHECK ("publisher_delivery_class" IN ('platform-plugin', 'hot-application') AND "owner_delivery_class" = "publisher_delivery_class" AND "owner_extension_id" = "publisher_extension_id" AND "owner_generation" BETWEEN 1 AND 9007199254740991 AND "template_version" BETWEEN 1 AND 9007199254740991 AND "template_id" LIKE substring("publisher_extension_id" FROM position('.' IN "publisher_extension_id") + 1) || '.%'),
      ...
      CREATE TABLE "k_nex_permission_catalog_snapshots" (
        ...
        CONSTRAINT "k_nex_permission_catalog_snapshots_owner_check" CHECK (
          ...
          OR ("owner_kind" IS NOT DISTINCT FROM 'extension' AND "owner_namespace" IS NULL AND "owner_delivery_class" IN ('platform-plugin', 'hot-application') AND "owner_extension_id" IS NOT NULL AND "owner_generation" BETWEEN 1 AND 9007199254740991)
        ),
      ...
      CREATE TABLE "k_nex_authorization_state" (
        "application_id" varchar(128) PRIMARY KEY NOT NULL,
        "authorization_revision" integer DEFAULT 0 NOT NULL,
        "lifecycle_revision" integer DEFAULT 0 NOT NULL,
        "updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
      ...
      CREATE TABLE "k_nex_owner_bootstrap_tokens" (
        "application_id" varchar(128) NOT NULL,
        "environment" varchar(64) NOT NULL,
        ...
        "created_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
        PRIMARY KEY ("application_id", "environment", "token_digest"),
        CONSTRAINT "k_nex_owner_bootstrap_tokens_digest_check" CHECK ("token_digest" ~ '^sha256:[0-9a-f]{64}$'),
        CONSTRAINT "k_nex_owner_bootstrap_tokens_consumed_check" CHECK ("consumed_at" IS NULL OR "consumed_at" >= "created_at")
      );
      CREATE UNIQUE INDEX "k_nex_owner_bootstrap_tokens_active_key" ON "k_nex_owner_bootstrap_tokens" ("application_id", "environment") WHERE "consumed_at" IS NULL;
    `);
  },
  async down({ db }: MigrateDownArgs): Promise<void> {
    await db.execute(sql`
      DROP TABLE "k_nex_owner_bootstrap_tokens";
