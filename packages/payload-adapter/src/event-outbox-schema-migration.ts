...
/** Clean-database v1 schema for transactional domain events and worker claims. */
export const kNexEventOutboxSchemaMigration = Object.freeze({
  name: "20260903_000005_knex_event_outbox",
  async up({ db }: MigrateUpArgs): Promise<void> {
    await db.execute(sql`
      ...
      CREATE INDEX "k_nex_outbox_claim_order_idx" ON "k_nex_outbox" ("status", "available_at", "id") WHERE "status"='pending';
      CREATE INDEX "k_nex_outbox_lease_recovery_idx" ON "k_nex_outbox" ("lease_expires_at", "id") WHERE "status"='processing' AND "lease_expires_at" IS NOT NULL;
      CREATE INDEX "k_nex_outbox_retention_cleanup_idx" ON "k_nex_outbox" ("retention_until", "id");
      CREATE INDEX "k_nex_outbox_correlation_idx" ON "k_nex_outbox" ("correlation_id");
      CREATE UNIQUE INDEX "k_nex_outbox_idempotency_scope_key" ON "k_nex_outbox" ("application_id", "plugin_id", "event_type", "idempotency_key") WHERE "idempotency_key" IS NOT NULL;
    `);
  },
  async down({ db }: MigrateDownArgs): Promise<void> {
    await db.execute(sql`DROP TABLE "k_nex_outbox";`);
  }
});
