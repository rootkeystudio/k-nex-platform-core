...
export interface VerifiedHotApplicationArtifactStage {
...
interface ArtifactRow {
  ...
  delivery_class: "hot-application";
  extension_id: string;
  version: string;
  runtime_abi: string;
}
...
interface BindingRow {
  ...
  activation_json: VerifiedHotApplicationArtifactStage["activation"];
  version: string;
}
...
function assertAuthority(stage: VerifiedHotApplicationArtifactStage, verified: VerifiedArtifact): void {
  ...
  if (verified.manifest.deliveryClass !== "hot-application" || owner.deliveryClass !== "hot-application" || verification.deliveryClass !== "hot-application" ||
    ...
    fail("ARTIFACT_INVALID", "Verified artifact authority does not bind the immutable Hot Application release.");
  }
}
...
export class PostgresVerifiedArtifactStore implements DurableDynamicArtifactStore, VerifiedRemoteUiArtifactReader {
  ...
  async stage(input: VerifiedHotApplicationArtifactStage): Promise<DurableDynamicArtifact> {
    const verified = await this.verifier.verify(input.verification);
    assertAuthority(input, verified);
    const manifest = verified.manifest;
    if (manifest.deliveryClass !== "hot-application") fail("ARTIFACT_INVALID", "Verified artifact is not a Hot Application.");
    ...
    await this.transaction(async (session) => {
      await session.query(
        ...
        [verified.artifactDigest, canonicalJson(input.verification.catalog), artifact, provenance, "hot-application", manifest.id, manifest.version, manifest.runtimeAbi]
      );
      await session.query(
        ...
        [input.owner.applicationId, input.owner.environment, "hot-application", input.owner.extensionId, input.owner.generationId, verified.artifactDigest,
    ...
    if (!stored || !same(stored.authority_json, input.authority) || !same(stored.activation_json, input.activation) || stored.version !== manifest.version) {
      fail("ARTIFACT_CONFLICT", "Immutable Hot Application generation is already bound to different verified content.");
    }
    ...
    return this.durable(stored, reverified);
  }
  ...
  async resolve(input: Readonly<{ owner: VerifiedGenerationAuthorityOwner; generationId: string; artifactDigest: string }>): Promise<DurableDynamicArtifact | undefined> {
    if (input.owner.deliveryClass !== "hot-application") return undefined;
    ...
    return verified ? this.durable(binding, verified) : undefined;
  }
  ...
  private async durable(binding: BindingRow, verified: VerifiedArtifact): Promise<DurableDynamicArtifact> {
    ...
    if (authority.artifactDigest !== binding.artifact_digest || authority.extensionId !== binding.extension_id || authority.generationId !== binding.generation_id ||
      ...
      authority.sbomDigest !== verified.entry.sbomDigest || authority.sourceCommit !== verified.entry.source.commit || verified.manifest.version !== binding.version) {
      fail("ARTIFACT_INVALID", "Stored Hot Application binding no longer matches its reverified artifact bytes.");
    }
