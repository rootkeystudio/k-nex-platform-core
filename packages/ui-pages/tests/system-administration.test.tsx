...
describe("system administration pages", () => {
  it("renders the eight fixed administration routes from server-produced view models", () => {
    const pages = [
      ...
      <SystemRoleDetailPage view={{ title: "Role", roleLabel: "Owner", roleState: "protected", activePermissionGroups: [{ owner: "Platform system", resources: [{ resource: "roles", operations: [{ operation: "manage", permissions: [{ label: "system.roles.manage", add: formAction("Add permission", "/api/system/access/roles/owner/permissions") }] }] }] }], templates: [{ id: "sales-manager", title: "Sales manager", instantiate: formAction("Instantiate template", "/api/system/access/templates/instantiate"), copySelected: formAction("Copy template permissions", "/api/system/access/templates/copy") }], inactiveDiagnostics: [{ id: "retired", label: "sales.pipeline.manage", state: "inactive-generation-retired", detail: "The retired generation cannot authorize." }], save: action }} />,
      ...
      <SystemAssignmentsPage view={{ title: "Assignments", assignments: [{ id: "assignment", principal: "user:alex", role: "Owner", state: "active", revision: "42", revoke: formAction("Revoke assignment", "/api/system/access/assignments/assignment/revoke") }], createAssignment: formAction("Create assignment", "/api/system/access/assignments"), manageAssignments: formAction("Manage assignments", "/api/system/access/assignments/manage") }} />,
    ...
    expect(markup).toContain('action="/api/system/access/roles/owner/permissions"');
