...
const acceptedContractDigest = "sha256:2b0ab3f5dcab0fba56f85ef0433b77502f2ea0564ab1c54fae4450551633cfa5";
...
export function validatePhase13ProductContract(value: unknown): RepositoryDiagnostic[] {
  ...
  if ([dataSemantics?.money, dataSemantics?.calendarDate, dataSemantics?.instant, dataSemantics?.reportingTimezone].some((value) => typeof value !== "string" || value === "") || retention?.defaultDeletion !== "soft-archive" || retention?.importExportArtifactsDays !== 30 || [retention?.domainRows, retention?.mergeLineage, retention?.audit, retention?.attachmentBytes].some((value) => typeof value !== "string" || value === "") || nonGoals.length < 10 || !nonGoals.includes("second-vertical") || !nonGoals.includes("public-cms") || duplicateIds(nonGoals).length > 0) diagnostics.push(diagnostic("PHASE13_PRODUCT_CONTRACT_INVALID", "$/dataSemantics", "Data semantics, retention, and non-goals must be closed, concrete, and preserve CRM-only scope.", "Restore complete product boundary semantics."));
