...
import { canonicalJson, DataSourceDescriptorSchema, PluginPageTemplateDescriptorSchema, PluginRouteDescriptorSchema, PluginUiContributionDescriptorSchema } from "@k-nex/contracts";
...
const acceptedContractDigest = "sha256:66cd82be8d87f4e791df3f1adbe58b9ff12381c8d03d1068b51942aab83c17b2";
...
export function validatePhase13ProductContract(value: unknown): RepositoryDiagnostic[] {
  ...
  try {
    ...
    if (actualDigest !== acceptedContractDigest) diagnostics.push(diagnostic("PHASE13_PRODUCT_CONTRACT_DRIFT", "$", `Contract digest ${actualDigest} differs from the accepted P13.4 digest.`, "Review the semantic change and update the accepted digest only with an accepted product-contract decision."));
  } catch (error) {
