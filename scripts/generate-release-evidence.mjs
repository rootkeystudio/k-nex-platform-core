...
const packageManifest = JSON.parse(readFileSync(resolve(fixtureRoot, "package.json"), "utf8"));
const sha256 = (content) => `sha256:${createHash("sha256").update(content).digest("hex")}`;
const componentVersion = (version) => version.replace(/^workspace:/u, "");
const sbom = createCycloneDxSbom(customer, Object.entries(packageManifest.dependencies).map(([name, version]) => ({ name, version: componentVersion(version) })));
