...
const unitProofs = [
  ...
  vitest("operator-adapter", "@k-nex/payload-adapter", ["tests/administration-operator-client.test.ts", "tests/administration-operator-https-server.test.ts", "tests/administration-extension-command-handler.test.ts", "tests/remote-administration-extension-operator.test.ts", "tests/runtime-extension-store.test.ts", "tests/static-host-runtime-inventory.test.ts"], [
    ...
    "rejects another actor before binding an execution request, then permits the planned actor",
    "leaves no execution binding when lifecycle mutation fails, so a restarted retry completes once",
    "rejects a completed operation whose execute digest was not committed atomically",
    ...
    "keeps an execute-command digest non-durable until terminal mutation",
    "commits the execute-command digest with terminal receipt, audit, and outbox state",
...
const processMarkers = [
  ...
  "P12_ADMINISTRATION_OPERATOR_CROSS_ACTOR_EXECUTE_POSTGRES_MTLS_HTTP_DENIED=PASS",
  ...
  "P12_UNRELATED_HOT_APPLICATION_LIFECYCLE_ADVANCE_PRESERVES_SALES_POSTGRES_HTTP=PASS",
