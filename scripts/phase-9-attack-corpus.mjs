...
const proofs = [
  ...
  vitestProof("plugin-manager", "@k-nex/runtime", "tests/plugin-manager.test.ts", [
    "PluginManager prepares Platform Plugin source/build authority only after impact planning, then resumes without duplicate requests",
...
const scenarios = [
  ...
  { id: "SCN-14", attack: "runtime DB-authored static graph", expected: "require source authority", evidence: [exactEvidence("plugin-manager", "PluginManager prepares Platform Plugin source/build authority only after impact planning, then resumes without duplicate requests"), exactEvidence("plugin-manager", "PluginManager rejects planner mismatches and unverified inventory authority")] },
