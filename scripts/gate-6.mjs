...
export async function validateGate6(root = repositoryRoot) {
  ...
  for (const marker of ["all 13 evidence classes", "runtime 200", "Payload adapter 32", "conformance-plan 6"]) {
    assert.ok(result.includes(marker), `Phase 6 result is missing: ${marker}.`);
  }
  for (const stale of ["all 11 evidence classes", "runtime 175", "runtime 178", "runtime 189", "Payload adapter 31", "conformance-plan 2", "conformance-plan 4", "conformance-plan 5", "this closeout commit"]) {
