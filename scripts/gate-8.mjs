...
// The 1.0.0 closure this gate verifies is immutable, and the current compiler
// emits a host that imports package exports 1.0.0 does not provide. Generating
// against this release would produce an application that cannot run its own
// migrations, so the factory must refuse it; a 1.0.0 application comes from the
// 1.0.0 compiler, which is how the packed customer boot proof produces one.
...
}), /must be generated by the 1\.0\.0 compiler/u, "The current compiler must refuse to generate an application pinned to the immutable 1.0.0 release.");
