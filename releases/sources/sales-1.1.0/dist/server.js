 ...
 */
export function issueSalesMergeRelationCapability(input) {
    if (typeof input.lineageId !== "string" || input.lineageId.length < 1 || input.lineageId.length > 128 || !/^[a-zA-Z0-9][a-zA-Z0-9._:-]*$/u.test(input.lineageId) ||
        ...
        !Number.isSafeInteger(input.authorizationRevision) || input.authorizationRevision < 1 || input.authorizationRevision > 1_000_000_000)
        throw new SalesDataMovementError("ACTION_FORBIDDEN");
    const relationIds = salesMergeRelationDescriptors.filter((descriptor) => descriptor.targetObjectType === input.targetObjectType).map(({ relationId }) => relationId);
    const body = Object.freeze({ kind: "sales.merge.relation-rewrite", lineageId: input.lineageId, applicationId: input.applicationId, environment: input.environment, targetObjectType: input.targetObjectType,
...
export function assertSalesMergeRelationCapability(capability, demand) {
    const descriptor = salesMergeRelationDescriptors.find(({ relationId }) => relationId === demand.relationId);
    if (capability === null || typeof capability !== "object" || capability.kind !== "sales.merge.relation-rewrite" || descriptor === undefined)
        throw new SalesDataMovementError("ACTION_FORBIDDEN");
    ...
    if (reissued.capabilityDigest !== capability.capabilityDigest || !sameStrings(reissued.relationIds, capability.relationIds) || !capability.relationIds.includes(demand.relationId) ||
        ...
        demand.fromRelatedRecordId !== String(capability.loserId) || demand.toRelatedRecordId !== String(capability.winnerId))
        throw new SalesDataMovementError("ACTION_FORBIDDEN");
    return descriptor;
}
...
export function createSalesMergeRelationAuditTransition(input) {
    ...
    if (!Number.isSafeInteger(input.preRevision) || input.preRevision < 1 || input.preRevision >= 1_000_000_000)
        throw new SalesDataMovementError("ACTION_FORBIDDEN");
...
async function crmTable(context, sourceId) {
    ...
    const result = await request.payload.find({ collection: spec.collection, depth: 0, overrideAccess: true, pagination: true, page: context.query.page.number, limit: detail ? 1 : context.query.page.size, select: { id: true, ...Object.fromEntries(selected.filter((field) => sourceId !== "sales.opportunity.detail" || !projectedOpportunityFields.has(field)).map((field) => [spec.storage[field], true])), ...(sourceId === "sales.opportunity.detail" ? { pipelineId: true, stageId: true } : {}), ...(selected.includes("amount") ? { currency: true } : {}) }, sort: ["id"], where, ...(user === undefined ? {} : { user }), req: request });
    ...
    if (sourceId === "sales.opportunity.detail" && documents.length === 1) {
        ...
        const stage = stages.docs[0];
        const semantic = trustedPipelineStage(stage, identity.applicationId, identity.environment, pipelineId);
        if (pipelines.docs.length !== 1 || stages.docs.length !== 1 || !Number.isSafeInteger(pipeline?.revision) || semantic === undefined || typeof stage?.name !== "string" || stage.name.length === 0 || !Number.isSafeInteger(stage.revision))
            throw new DataSourceGatewayError("INVALID_SOURCE_OUTPUT", 500, "Sales opportunity pipeline projection is incomplete.");
...
export const salesTimelineHandler = async (context) => {
    ...
    const notes = directTargetAuthorized ? await timelineFind(() => request.payload.find({ ...options, collection: "sales-notes", sort: ["-occurredAt", "-id"], select: { id: true, status: true, occurredAt: true, revision: true, ...(selected.includes("body") ? { body: true } : {}) } })) : undefined;
...
export const salesTaskCreateHandler = async ({ actor, request, authorizationContext, input, idempotencyKey, signal }) => {
    ...
    if (!isSalesRecordId(resourceId))
        throw new ActionGatewayError("ACTION_FORBIDDEN", 403, "Sales task identity is invalid.");
    const eventId = durableActionEventId(authorization, idempotencyKey);
    const occurredAt = new Date().toISOString();
...
async function currentOpportunityStageSemantic(payloadRequest, authorization, document, user) {
    const pipelineId = Number(persistedWorkflowId(document.pipelineId, "opportunity pipeline ID"));
    const stageId = workflowState(document, "stageId");
    ...
    const semantic = found.docs.length === 1 ? trustedPipelineStage(found.docs[0], authorization.applicationId, authorization.environment, pipelineId) : undefined;
    if (semantic === undefined)
        throw new ActionGatewayError("STALE_RECORD", 409, "Sales opportunity stage is unavailable.");
    return semantic;
}
...
export const salesWorkflowActionHandler = async ({ actor, request, authorizationContext, input, idempotencyKey, signal }) => {
    ...
    if (actionId === "sales.opportunity.update" && (opportunitySemantic === "won" || opportunitySemantic === "lost"))
