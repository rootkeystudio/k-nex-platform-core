...
function initialWorkflowValues(input, actionId, fields) {
    ...
    return Object.fromEntries(fields.map((field) => {
        ...
        if (actionId === "sales.opportunity.close" && field === "stage")
            return [field, closeStageOptions(cellText(row?.values["stage-semantic"]))[0]?.id ?? ""];
        if (actionId === "sales.opportunity.stage.update" && field === "stage") {
            const current = cellText(row?.values["stage-semantic"]);
            return [field, opportunityTransitionTarget[current]?.[0] ?? ""];
        }
...
function closeStageOptions(stageSemantic) {
    if (["qualification", "discovery", "proposal"].includes(stageSemantic))
        return [{ id: "lost", label: "Lost" }];
    if (stageSemantic === "negotiation")
        return [{ id: "won", label: "Won" }, { id: "lost", label: "Lost" }];
    return [];
}
                ...
                : (actionId === "sales.opportunity.update" && ["primaryContactMode", "amountMode", "expectedCloseDateMode"].includes(field) || (actionId === "sales.contact.update" || actionId === "sales.lead.update") && (field === "emailMode" || field === "phoneMode"))
                    ...
                    : actionId === "sales.opportunity.stage.update" && field === "stage"
                        ...
                        : actionId === "sales.opportunity.close" && field === "stage"
                            ? createElement(Select, { key: field, name: field, label: fieldLabels[field] ?? field, value: values[field] ?? "", required: true, options: closeStageOptions(cellText(projected?.values["stage-semantic"])), onChange: (value) => setValues((current) => ({ ...current, stage: value })) })
...
function detailActionAllowed(input, record) {
    ...
    if (sourceId === salesOpportunityDetailDescriptor.id && actionId === "sales.opportunity.close" && closeStageOptions(stageSemantic).length === 0)
        return false;
    if (sourceId === salesOpportunityDetailDescriptor.id && stageSemantic === "negotiation" && actionId === "sales.opportunity.stage.update")
        return false;
    if (sourceId === salesOpportunityDetailDescriptor.id && (stageSemantic === "won" || stageSemantic === "lost"))
