...
export const salesRelatedRecordTypes = Object.freeze(["sales.account", "sales.contact", "sales.lead", "sales.opportunity", "sales.task"]);
...
export function salesCommonFields(options) {
    return [
        ...
        ...(options.lifecycle === undefined ? [] : [{ name: "status", type: "select", required: true, defaultValue: options.lifecycle[0], options: options.lifecycle.map((value) => ({ label: value, value })), access: protectedFieldAccess }])
    ];
}
function collection(slug, fields, indexes) {
    return { slug, access: directPayloadAccess, fields, indexes };
}
const activitySupersedesField = {
...
//# sourceMappingURL=crm-core.js.map