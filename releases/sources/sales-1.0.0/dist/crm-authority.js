...
export const salesCrmPermissionDescriptors = Object.freeze(salesCrmPermissionIds.map(descriptor));
...
const matrixActionPermissions = new Map();
for (const row of salesCrmObjectFieldActionMatrix) {
    for (const permissionId of [row.readPermissionId, row.writePermissionId, "archivePermissionId" in row ? row.archivePermissionId : undefined, ...Object.values("sensitiveFields" in row ? row.sensitiveFields : {})]) {
        if (permissionId !== undefined)
            permissionPolicies.set(permissionId, row.recordPolicyId);
    }
    for (const [actionId, permissionId] of Object.entries("actionPermissions" in row ? row.actionPermissions : {})) {
        ...
        if (prior !== undefined && prior !== permissionId)
            throw new TypeError(`Sales action ${actionId} has conflicting frozen permissions.`);
        ...
        if (!applicationPermission.has(permissionId))
            permissionPolicies.set(permissionId, row.recordPolicyId);
    }
}
...
const recordId = { type: "string", minLength: 1, maxLength: 10 };
const principalIdentity = { type: "string", minLength: 1, maxLength: 160 };
const actionOutput = {
    type: "object",
    properties: {
        id: recordId,
...
const actionInput = { type: "object", properties: {}, additionalProperties: false };
const revision = { type: "integer", minimum: 1 };
const configurationRevision = { type: "integer", minimum: 0 };
const shortText = { type: "string", minLength: 1, maxLength: 256 };
const optionalText = { type: "string", minLength: 1, maxLength: 256 };
export const salesPhoneMaxLength = 64;
const phoneText = { type: "string", minLength: 1, maxLength: salesPhoneMaxLength };
const mediaType = { type: "string", minLength: 1, maxLength: 128 };
const relatedRecordType = { type: "string", enum: ["sales.account", "sales.contact", "sales.lead", "sales.opportunity", "sales.task"] };
const ownershipRecordType = { type: "string", enum: ["sales.account", "sales.contact", "sales.lead", "sales.opportunity"] };
const activityType = { type: "string", enum: ["call", "meeting", "email"] };
const opportunityStage = { type: "string", enum: ["qualification", "discovery", "proposal", "negotiation", "won", "lost"] };
const qualificationMode = { type: "string", enum: ["create", "link"] };
const optionalMutationMode = { type: "string", enum: ["retain", "set", "clear"] };
const emailProviderId = { type: "string", enum: ["email.reference.v1"] };
const calendarProviderId = { type: "string", enum: ["calendar.reference.v1"] };
const utcTimestamp = { type: "string", minLength: 24, maxLength: 24 };
const reminderReferenceKind = { type: "string", enum: ["task", "activity"] };
const communicationRecordType = { type: "string", enum: ["sales.contact", "sales.lead"] };
export const salesCalendarDatePattern = "^(?:000[1-9]|00[1-9][0-9]|0[1-9][0-9]{2}|[1-9][0-9]{3})-[0-9]{2}-[0-9]{2}$";
export function isSalesCalendarDate(value) {
...
const calendarDate = { type: "string", minLength: 10, maxLength: 10, description: "Exact Gregorian YYYY-MM-DD date in years 0001 through 9999." };
const reportId = { type: "string", enum: ["sales.report.pipeline-value-by-stage", "sales.report.weighted-forecast", "sales.report.won-lost-conversion", "sales.report.lead-conversion", "sales.report.activity-by-owner-team", "sales.report.task-aging", "sales.report.sales-cycle-duration"] };
const reportWindowMode = { type: "string", enum: ["as-of", "current-reporting-week", "previous-complete-reporting-week", "current-reporting-month", "previous-complete-reporting-month"] };
const weeklyWeekday = { type: "integer", minimum: 1, maximum: 7 };
const weeklyLocalTime = { type: "string", minLength: 5, maxLength: 5 };
const opportunityMoney = { type: "object", properties: {
        kind: { type: "string", enum: ["money"] }, value: { type: "string", minLength: 1, maxLength: 128 },
        currency: { type: "string", minLength: 3, maxLength: 3 }, scale: { type: "integer", minimum: 0, maximum: 18 }
    }, required: ["kind", "value", "currency", "scale"], additionalProperties: false };
const stageUuid = { type: "string", minLength: 36, maxLength: 36 };
const stringArray = { type: "array", items: { type: "string" }, maxItems: 6 };
const pipelineStageSnapshot = { type: "object", properties: {
        stageId: stageUuid, expectedRevision: revision, semantic: opportunityStage, name: shortText,
        position: { type: "integer", minimum: 0, maximum: 5 }, probabilityBasisPoints: { type: "integer", minimum: 0, maximum: 10_000 },
        allowedTransitionStageIds: stringArray, requiredFieldIds: { type: "array", items: { type: "string" }, maxItems: 1 }
    }, required: ["stageId", "expectedRevision", "semantic", "name", "position", "probabilityBasisPoints", "allowedTransitionStageIds", "requiredFieldIds"], additionalProperties: false };
const pipelineStageOutput = { type: "object", properties: {
        stageId: stageUuid, revision, semantic: opportunityStage, name: shortText,
        position: { type: "integer", minimum: 0, maximum: 5 }, probabilityBasisPoints: { type: "integer", minimum: 0, maximum: 10_000 },
        allowedTransitionStageIds: stringArray, requiredFieldIds: { type: "array", items: { type: "string" }, maxItems: 1 }, status: { type: "string", enum: ["active"] }
    }, required: ["stageId", "revision", "semantic", "name", "position", "probabilityBasisPoints", "allowedTransitionStageIds", "requiredFieldIds", "status"], additionalProperties: false };
const savedViewName = { type: "string", minLength: 1, maxLength: 120, maxUtf8Bytes: 120 };
const savedViewVisibility = {
    oneOf: [
        { type: "object", properties: { kind: { type: "string", enum: ["personal"] } }, required: ["kind"], additionalProperties: false },
        { type: "object", properties: { kind: { type: "string", enum: ["team"] }, teamId: { type: "string", minLength: 1, maxLength: 120, maxUtf8Bytes: 120 } }, required: ["kind", "teamId"], additionalProperties: false }
    ]
};
const savedViewFilterValue = {
    oneOf: [
        { type: "null" },
        { type: "string", maxLength: 512, maxUtf8Bytes: 512 },
        { type: "integer", minimum: -Number.MAX_SAFE_INTEGER, maximum: Number.MAX_SAFE_INTEGER },
        { type: "array", items: { type: "string", maxLength: 512, maxUtf8Bytes: 512 }, minItems: 1, maxItems: 20 },
        { type: "array", items: { type: "integer", minimum: -Number.MAX_SAFE_INTEGER, maximum: Number.MAX_SAFE_INTEGER }, minItems: 1, maxItems: 20 }
    ]
};
const savedViewDefinition = { type: "object", properties: {
        kind: { type: "string", enum: ["table", "kanban", "calendar"] }, targetObjectId: { type: "string" },
        source: { type: "object", properties: { id: { type: "string" }, version: revision, sourceSchema: { type: "object", properties: { id: { type: "string" }, version: revision }, required: ["id", "version"], additionalProperties: false }, structuralCompatibilityHash: { type: "string" } }, required: ["id", "version", "sourceSchema", "structuralCompatibilityHash"], additionalProperties: false },
        fields: { type: "array", items: { type: "string" }, minItems: 1, maxItems: 8 },
        filters: { type: "array", items: { type: "object", properties: { fieldId: { type: "string" }, operator: { type: "string" }, value: savedViewFilterValue }, required: ["fieldId", "operator"], additionalProperties: false }, maxItems: 8 },
        sorts: { type: "array", items: { type: "object", properties: { fieldId: { type: "string" }, direction: { type: "string", enum: ["asc", "desc"] } }, required: ["fieldId", "direction"], additionalProperties: false }, maxItems: 2 },
        grouping: { type: "string" }, dateField: { type: "string" }, calendarRange: { type: "object", properties: { start: { type: "string" }, end: { type: "string" }, timezone: { type: "string" } }, required: ["start", "end", "timezone"], additionalProperties: false },
        presentation: { type: "object", properties: { density: { type: "string" }, mode: { type: "string" } }, additionalProperties: false }, pageSize: { type: "integer", minimum: 1, maximum: 100 }
    }, required: ["kind", "targetObjectId", "source", "fields", "filters", "sorts", "presentation", "pageSize"], additionalProperties: false };
const mutationInput = (properties, required) => ({ type: "object", properties, required, additionalProperties: false });
const safeId = { type: "integer", minimum: 1, maximum: Number.MAX_SAFE_INTEGER };
const sha256Digest = { type: "string", minLength: 71, maxLength: 71 };
const importMapping = (fields, minimum) => ({ type: "array", minItems: minimum, maxItems: fields.length, items: { type: "object", properties: { header: { type: "string", minLength: 1, maxLength: 120, maxUtf8Bytes: 120 }, fieldId: { type: "string", enum: [...fields] } }, required: ["header", "fieldId"], additionalProperties: false } });
const importRequest = (targetObjectType, fields, minimum) => ({ type: "object", properties: { uploadArtifactId: { type: "string", minLength: 1, maxLength: 128 }, targetObjectType: { type: "string", enum: [targetObjectType] }, columnMapping: importMapping(fields, minimum), expectedAuthorizationRevision: safeId }, required: ["uploadArtifactId", "targetObjectType", "columnMapping", "expectedAuthorizationRevision"], additionalProperties: false });
const exportRequest = (targetObjectType, sourceId, fields) => ({ type: "object", properties: { targetObjectType: { type: "string", enum: [targetObjectType] }, sourceId: { type: "string", enum: [sourceId] }, sourceVersion: { type: "integer", enum: [1] }, sourceSchemaVersion: { type: "integer", enum: [1] }, selectedFields: { type: "array", minItems: 1, maxItems: fields.length, items: { type: "string", enum: [...fields] } }, expectedAuthorizationRevision: safeId }, required: ["targetObjectType", "sourceId", "sourceVersion", "sourceSchemaVersion", "selectedFields", "expectedAuthorizationRevision"], additionalProperties: false });
const dataMovementInputs = Object.freeze({
...
const workflowActionInputs = Object.freeze({
    ...dataMovementInputs,
    ...
    "sales.email.send": mutationInput({ providerId: emailProviderId, relatedRecordType: communicationRecordType, relatedRecordId: recordId, subject: shortText, body: { type: "string", minLength: 1, maxLength: 10_000 } }, ["providerId", "relatedRecordType", "relatedRecordId", "subject", "body"]),
    ...
    "sales.integration.configure": mutationInput({ providerId: { type: "string", enum: ["email.reference.v1", "calendar.reference.v1"] }, expectedRevision: configurationRevision, operation: { type: "string", enum: ["activate", "revoke"] } }, ["providerId", "expectedRevision", "operation"]),
    "sales.report.run": mutationInput({ reportId, windowMode: reportWindowMode }, ["reportId", "windowMode"]),
    "sales.report.schedule": mutationInput({ operation: { type: "string", enum: ["upsert", "cancel"] }, reportId, recipientId: principalIdentity, expectedRevision: configurationRevision, weekday: weeklyWeekday, localTime: weeklyLocalTime, windowMode: reportWindowMode }, ["operation", "reportId", "recipientId", "expectedRevision"]),
    "sales.note.create": mutationInput({ relatedRecordType, relatedRecordId: recordId, body: { type: "string", minLength: 1, maxLength: 10_000 }, replacesNoteId: recordId }, ["relatedRecordType", "relatedRecordId", "body"]),
    "sales.attachment.link": mutationInput({ relatedRecordType, relatedRecordId: recordId, storageReference: shortText, filename: shortText, mediaType, byteSize: { type: "integer", minimum: 0, maximum: 1_073_741_824 } }, ["relatedRecordType", "relatedRecordId", "storageReference", "filename", "mediaType", "byteSize"]),
    "sales.attachment.remove": mutationInput({ id: recordId, expectedRevision: revision }, ["id", "expectedRevision"]),
    "sales.pipeline.update": mutationInput({ id: recordId, expectedRevision: revision, name: shortText, orderedStageIds: stringArray, stages: { type: "array", items: pipelineStageSnapshot, minItems: 6, maxItems: 6 } }, ["id", "expectedRevision", "name", "orderedStageIds", "stages"]),
...
const ownershipOutput = { type: "object", properties: { recordType: ownershipRecordType, id: recordId, revision, ownerId: principalIdentity, teamId: principalIdentity }, required: ["recordType", "id", "revision", "ownerId"], additionalProperties: false };
...
function workflowOutput(id) {
    if (id === "sales.report.run")
        return mutationInput({ reportRunId: { type: "string", minLength: 3, maxLength: 128 }, state: { type: "string", enum: ["queued"] }, revision }, ["reportRunId", "state", "revision"]);
    if (id === "sales.report.schedule")
        return mutationInput({ reportId, recipientId: principalIdentity, state: { type: "string", enum: ["active", "cancelled"] }, revision }, ["reportId", "recipientId", "state", "revision"]);
    if (id === "sales.integration.configure")
        return mutationInput({ providerId: { type: "string", enum: ["email.reference.v1", "calendar.reference.v1"] }, revision: { type: "integer", minimum: 1 }, status: { type: "string", enum: ["accepted"] } }, ["providerId", "revision", "status"]);
    if (id === "sales.import.dry-run")
        return mutationInput({ importJobId: safeId, revision: { type: "integer", enum: [2] }, state: { type: "string", enum: ["validated"] }, uploadDigest: sha256Digest, acceptedRows: { type: "integer", minimum: 0, maximum: 10_000 }, rejectedRows: { type: "integer", minimum: 0, maximum: 10_000 }, diagnosticDigest: sha256Digest }, ["importJobId", "revision", "state", "uploadDigest", "acceptedRows", "rejectedRows", "diagnosticDigest"]);
    if (id === "sales.import.commit")
        return mutationInput({ importJobId: safeId, revision: { type: "integer", minimum: 3, maximum: Number.MAX_SAFE_INTEGER }, state: { type: "string", enum: ["queued"] }, receiptId: { type: "string", minLength: 1, maxLength: 128 } }, ["importJobId", "revision", "state", "receiptId"]);
    if (id === "sales.import.cancel")
        return mutationInput({ importJobId: safeId, revision: { type: "integer", minimum: 2, maximum: Number.MAX_SAFE_INTEGER }, state: { type: "string", enum: ["cancelled"] } }, ["importJobId", "revision", "state"]);
    if (id === "sales.export.create")
        return mutationInput({ exportJobId: safeId, revision: { type: "integer", enum: [1] }, state: { type: "string", enum: ["queued"] }, snapshotDigest: sha256Digest, snapshotRevision: safeId, receiptId: { type: "string", minLength: 1, maxLength: 128 } }, ["exportJobId", "revision", "state", "snapshotDigest", "snapshotRevision", "receiptId"]);
    if (id === "sales.export.cancel")
        return mutationInput({ exportJobId: safeId, revision: { type: "integer", minimum: 2, maximum: Number.MAX_SAFE_INTEGER }, state: { type: "string", enum: ["cancelled"] } }, ["exportJobId", "revision", "state"]);
    if (id === "sales.merge.commit")
        return mutationInput({ winnerId: safeId, winnerRevision: { type: "integer", minimum: 2, maximum: Number.MAX_SAFE_INTEGER }, loserId: safeId, loserRevision: { type: "integer", minimum: 2, maximum: Number.MAX_SAFE_INTEGER }, matchKind: { type: "string", enum: ["account-name", "contact-email", "contact-phone", "contact-email-and-phone"] }, lineageId: { type: "string", minLength: 1, maxLength: 128 }, lineageDigest: sha256Digest, rewrittenRelationCounts: { type: "array", minItems: 6, maxItems: 7, items: { type: "object", properties: { relationId: { type: "string", enum: ["sales_contacts.account_id", "sales_opportunities.account_id", "sales_leads.qualified_account_id", "sales_activities.related_record_id where related_record_type=sales.account", "sales_notes.related_record_id where related_record_type=sales.account", "sales_attachment_references.related_record_id where related_record_type=sales.account", "sales_tasks.related_record_id where related_record_type=sales.account", "sales_opportunities.primary_contact_id", "sales_leads.qualified_contact_id", "sales_activities.related_record_id where related_record_type=sales.contact", "sales_notes.related_record_id where related_record_type=sales.contact", "sales_attachment_references.related_record_id where related_record_type=sales.contact", "sales_tasks.related_record_id where related_record_type=sales.contact"] }, count: { type: "integer", minimum: 0, maximum: Number.MAX_SAFE_INTEGER } }, required: ["relationId", "count"], additionalProperties: false } } }, ["winnerId", "winnerRevision", "loserId", "loserRevision", "matchKind", "lineageId", "lineageDigest", "rewrittenRelationCounts"]);
    const statuses = workflowOutputStatuses[id];
    if (statuses === undefined)
        return actionOutput;
    const qualification = id === "sales.lead.qualify";
    if (id === "sales.opportunity.create" || id === "sales.opportunity.stage.update" || id === "sales.opportunity.close")
        return {
    ...
    if (id === "sales.saved-view.create" || id === "sales.saved-view.update")
        return {
...
export const salesCrmActionDescriptors = Object.freeze(Object.entries(actionPermissions).map(([id, permission]) => {
    const policy = actionPolicies.get(id) ?? operationActionPolicies[id];
    if (policy === undefined)
        throw new TypeError(`Sales action ${id} has no policy.`);
    ...
    return {
        ...
        outputSchema: id === "sales.ownership.assign" ? ownershipOutput : workflow === undefined ? { type: "object", properties: { accepted: { type: "boolean" } }, required: ["accepted"], additionalProperties: false } : workflowOutput(id),
        permission,
        policy,
        effect: id === "sales.merge.commit" ? "destructive" : "write",
...
function routeDescriptor(id, permission) {
...
export const salesCrmRouteDescriptors = Object.freeze(Object.entries(routePermissions).map(([id, permission]) => routeDescriptor(id, permission)));
export const salesCrmRoutePermissions = routePermissions;
//# sourceMappingURL=crm-authority.js.map